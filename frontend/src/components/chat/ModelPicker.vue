    ...
    <div class="flex flex-col">
      ...
      <div class="p-2">
        ...
        <div v-if="showModelList" class="mt-1">
          ...
          <p
            v-else-if="!loading && filteredInstalled.length === 0 && filteredRecommended.length === 0"
            class="px-2 py-2 text-xs text-slate-500"
          >
            {{
              models.length === 0 && recommended.length === 0
                ? t('app.model_catalog_empty')
                : t('app.model_search_empty')
            }}
          </p>

          <div class="max-h-56 overflow-y-auto">
            <template v-if="filteredInstalled.length > 0">
              <p class="px-2 pt-1 pb-0.5 text-[10px] font-semibold tracking-wide text-slate-400 uppercase">
                {{ t('app.model_section_installed') }}
              </p>
              <button
              ...
              >
                <span class="min-w-0">
                  ...
                  >{{ modelTierLabel(model) }}</span>
                  <span
                    v-if="model.family && model.family !== 'unknown'"
                    class="block text-[10px] text-slate-400 capitalize"
                  >{{ model.family }}</span>
                </span>
                <span
            ...
            <template v-if="filteredRecommended.length > 0">
              <p class="px-2 pt-2 pb-0.5 text-[10px] font-semibold tracking-wide text-slate-400 uppercase">
                {{ t('app.model_section_recommended') }}
              </p>
              <div
                v-for="model in filteredRecommended"
                :key="`recommended-${model.catalog_id || model.id}`"
                class="rounded-lg px-2 py-2 opacity-70"
              >
                ...
                <p
                  ...
                  :title="t('app.model_quality_estimate')"
                >{{ modelTierLabel(model) }}</p>
                <p class="mt-0.5 text-[10px] leading-snug text-slate-400">
                  {{
                    t('app.model_pull_hint', {
                      command: `ollama pull ${model.ollama_pull || model.name}`,
                    })
                  }}
                </p>
              </div>
...
import type { ModelTier, OllamaModelInfo } from '@/types/api'
...
const recommended = ref<OllamaModelInfo[]>([])
...
const selectedLabel = computed(() => {
  ...
  const match = models.value.find((row) => row.name === selection.value)
  return match?.display_name || selection.value
})
...
function matchesQuery(row: OllamaModelInfo, needle: string): boolean {
  if (!needle) return true
  const hay = [
    row.name,
    row.display_name,
...
const filteredInstalled = computed(() => {
  const needle = query.value.trim().toLowerCase()
  return models.value.filter((row) => matchesQuery(row, needle))
})

const filteredRecommended = computed(() => {
  const needle = query.value.trim().toLowerCase()
  return recommended.value.filter((row) => matchesQuery(row, needle))
})
...
function setAuto(enabled: boolean) {
  ...
  const fallback = filteredInstalled.value[0]?.name || models.value[0]?.name
  if (fallback) {
...
function selectFirstResult() {
  if (hasSearch.value && autoMatchesSearch.value && filteredInstalled.value.length === 0) {
    setAuto(true)
    menuRef.value?.close()
    return
  }
  const first = filteredInstalled.value[0]
  if (first) choose(first.name)
}

function modelTierLabel(model: OllamaModelInfo): string {
  const tier = String(model.tier || '').toLowerCase() as ModelTier | ''
  if (tier === 'fast') return t('app.model_quality_fast')
  if (tier === 'balanced') return t('app.model_quality_balanced')
  if (tier === 'high') return t('app.model_quality_high')
  if (tier === 'best') return t('app.model_quality_best')
...
onMounted(async () => {
  ...
  try {
    ...
    recommended.value = catalog.recommended ?? []
    if (catalog.error) {
